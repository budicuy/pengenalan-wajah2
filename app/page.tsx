...
import { getDataset } from "@/app/action";
...
export default function Home() {
  ...
  const rebuildFaceMatcherWithThreshold = (
    descriptors: FaceApiType.LabeledFaceDescriptors[],
  ) => {
  ...
  const initSystem = async () => {
    try {
      ...
      // Dynamically import face-api only on the client side to avoid SSR errors
      if (!faceapi) {
      ...
      // Fetch list of students in dataset via Server Action
      ...
      let loadedDescriptors: FaceApiType.LabeledFaceDescriptors[] = [];
  ...
  const extractDescriptors = async (studentList: Student[]) => {
    const loadedDescriptors: FaceApiType.LabeledFaceDescriptors[] = [];
